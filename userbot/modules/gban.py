    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        try:
            ...
            return await event.edit(
                "`Terjadi Kesalahan... Mohon Lapor Ke ` @knsgnsn", str(err)
            )
            ...
            if i.sender == str(guser.id):
                ...
                if admin or creator:
                    try:
                        ...
                        await tele.reply(
                            f"** s** \n"
                            f"**s  :** [{guser.id}](tg://user?id={guser.id})\n"
                            f"** :** ``"
                        )
                    except BaseException:
                        return
...
async def gben(userbot):
    ...
    if sender.id != me.id:
        dark = await dc.reply("`s ...`")
    else:
        dark = await dc.edit("`ss s s  s..`")
    me = await userbot.client.get_me()
    await dark.edit(f"`s   s ..`")
